				...
				for r := startRow; r <= endRow; r++ {
					for c := startCol; c <= endCol; c++ {
						if r == startRow && c == startCol {
							...
							if r == endRow {
								s.wrapped.Put(r, c, tabextract.EndRowMerged, 0)
							} else {
								s.wrapped.Put(r, c, tabextract.ContinueRowMerged, 0)
							}
						} else if c == endCol {
							// first and last column are NOT the same
							s.wrapped.Put(r, c, tabextract.EndColumnMerged, 0)
						} else {
							s.wrapped.Put(r, c, tabextract.ContinueColumnMerged, 0)
						}
					}
				}
