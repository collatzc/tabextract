...
func (x *Formatter) Add(fmtID uint16, formatCode string) error {
	...
	if ok {
		return errors.New("tabextract/commonxl: cannot replace default number formats")
	}

	_, ok2 := x.customCodes[fmtID]
	if ok2 {
		return errors.New("tabextract/commonxl: cannot replace existing number formats")
	}
