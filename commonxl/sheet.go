...
func (s *Sheet) Put(row, col int, value interface{}, fmtNum uint16) {
	...
	if row >= s.NumRows || col >= s.NumCols {
		if tabextract.Debug {
			log.Printf("tabextract: cell out of bounds row %d>=%d, col %d>=%d",
	...
	if spec, ok := value.(string); ok {
		if spec == tabextract.EndRowMerged || spec == tabextract.EndColumnMerged || spec == tabextract.ContinueRowMerged || spec == tabextract.ContinueColumnMerged {
...
func (s *Sheet) Set(row, col int, value interface{}) {
	if row > s.NumRows || col > s.NumCols {
		log.Println("tabextract: cell out of bounds")
		return
	}
