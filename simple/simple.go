...
func (t *simpleFile) Get(name string) (tabextract.Collection, error) {
	return t, nil
}
...
func (t *simpleFile) Scan(args ...interface{}) error {
	...
	if len(row) != len(args) {
		return fmt.Errorf("tabextract/simple: expected %d Scan destinations, got %d", len(row), len(args))
	}
	...
	for i, a := range args {
		...
		case *string:
			*v = row[i]
		case *time.Time:
			return errors.New("tabextract/simple: time.Time not supported, you must parse date strings manually")
		default:
			return tabextract.ErrInvalidScanType
		}
		if err != nil {
			return err
		}
