...
var _ = tabextract.Register("csv", 15, OpenCSV)
...
func OpenCSV(filename string) (tabextract.Source, error) {
	...
	if err != nil {
		...
		case *csv.ParseError:
			return nil, tabextract.WrapErr(perr, tabextract.ErrNotInFormat)
		}
		if total < 10 {
			// probably? not in this format
			return nil, tabextract.WrapErr(err, tabextract.ErrNotInFormat)
		}
		return nil, err
	}
	...
	if looksGood == 1 {
		return t, tabextract.ErrNotInFormat
	}
